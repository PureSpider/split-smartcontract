...
usage () {
    ...
    echo "         --protocol <protocol> : optional, select target protocol - default is hangzhou"
    ...
    echo
    echo "   Protocols: delphi | edo | florence | granada | hangzhou | ithaca"
}
...
do
    if [[ "$arg" == --native ]]; then
        ...
        echo "== warning: Using Proto alpha for Ithaca =="
...
case $action in
    ...
    "regular" )
        ...
        if [[ $has_mockup == yes ]]; then
            ...
            ext/tezos-binaries/tezos-client \
                --protocol $protocol \
                --base-dir $MOCKUP \
                create mockup
